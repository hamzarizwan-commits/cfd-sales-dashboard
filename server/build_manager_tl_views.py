...
SELECT u.email                        AS email,
       COUNT(DISTINCT l.listing_nk)   AS active_listings,
       COUNT(DISTINCT a.account_sk)   AS active_agencies
FROM   podl_bayutsa.dim_accounts a
LEFT JOIN podl_bayutsa.fact_listings l
    ON l.account_sk = a.account_sk
   AND l.listing_status_sk = 'active'
JOIN   jarvis_empg.jarvis_empg__crm_clients cl
    ON CAST(cl.id AS VARCHAR) = a.account_nk
   AND cl.is_active = 1
   AND (cl.operation_type IS NULL OR cl.operation_type != 'delete')
JOIN   jarvis_empg.jarvis_empg__crm_users u
    ON u.id = cl.assignee_id
JOIN   jarvis_empg.jarvis_empg__crm_client_detail_summaries cds
    ON cds.client_id = cl.id
   AND (cds.operation_type IS NULL OR cds.operation_type != 'delete')
WHERE  u.email LIKE '%@bayut.sa'
  AND  (
      l.listing_nk IS NOT NULL
      OR (COALESCE(cds.listings_active, 0) = 0 AND cds.active_contracts_count > 0)
  )
GROUP BY u.email
""")
...
# Build per-consultant active listing / agency lookups
...
# Zero posters: paying clients (active contract) with zero active listings on the platform
print("  Querying zero-poster client counts per consultant...")
_zp_df2 = run_query("""
SELECT u.email                          AS email,
       COUNT(DISTINCT cds.client_id)    AS zero_poster_clients
FROM   jarvis_empg.jarvis_empg__crm_client_detail_summaries cds
JOIN   jarvis_empg.jarvis_empg__crm_clients cl
    ON cds.client_id = cl.id
   AND cl.is_active = 1
   AND (cl.operation_type IS NULL OR cl.operation_type != 'delete')
JOIN   jarvis_empg.jarvis_empg__crm_users u
    ON cl.assignee_id = u.id
WHERE  u.email LIKE '%@bayut.sa'
  AND  (cds.operation_type IS NULL OR cds.operation_type != 'delete')
  AND  COALESCE(cds.listings_active, 0) = 0
  AND  cds.active_contracts_count > 0
GROUP BY u.email
""")
_zero_posters = {}
for _, _zrow in _zp_df2.iterrows():
    _cnt = int(_zrow['zero_poster_clients'])
    if _cnt > 0:
        _zero_posters[_zrow['email']] = _cnt
print(f"  Zero-poster consultants: {len(_zero_posters)}, total zero-poster clients: {sum(_zero_posters.values())}")
...
# ============================================================
# ONLINE COLLECTIONS (online-user@jarvisempg.com channel)
# ============================================================
print("\nPulling online channel collections...")
online_coll_df = run_query(f"""
SELECT
    DATE_TRUNC('day', p.collected_at)::date as collection_date,
    SUM(p.net_amount) as daily_amount
FROM jarvis_empg.jarvis_empg__crm_payments p
JOIN jarvis_empg.jarvis_empg__crm_users u ON p.collected_by_id = u.id
WHERE u.email = 'online-user@jarvisempg.com'
  AND (p.operation_type IS NULL OR p.operation_type != 'delete')
  AND p.deleted_at IS NULL
  AND p.collected_at >= '2026-01-01' AND p.collected_at < '{eom_daily}'
GROUP BY collection_date
ORDER BY collection_date
""")

# Month num: 2026-01 = 1, 2026-02 = 2, etc.
online_coll_daily = {}
online_coll_monthly = {}
for _, row in online_coll_df.iterrows():
    d = str(row['collection_date'])
    amt = round(float(row['daily_amount']), 2)
    online_coll_daily[d] = amt
    # derive month_num from date
    y, mo = int(d[:4]), int(d[5:7])
    mn = (y - 2026) * 12 + mo  # Jan2026=1, Feb2026=2, ...
    online_coll_monthly[mn] = round(online_coll_monthly.get(mn, 0) + amt, 2)

with open('C:/Users/hamza.rizwan/online_collections_daily_2026.json', 'w') as f:
    json.dump(online_coll_daily, f, separators=(',', ':'))
with open('C:/Users/hamza.rizwan/online_collections_monthly_2026.json', 'w') as f:
    json.dump({str(k): v for k, v in online_coll_monthly.items()}, f, separators=(',', ':'))
print(f"  Online collections saved: {len(online_coll_daily)} days, {len(online_coll_monthly)} months, total={sum(online_coll_monthly.values()):.0f}")

# ============================================================
# ONLINE PACKAGES SOLD BY MONTH (named packages only, not credits)
# ============================================================
print("\nPulling online named packages sold by month...")
online_pkg_df = run_query(f"""
WITH months AS (
    {build_months_cte()}
)
SELECT m.month_num, COUNT(*) as packages_sold
FROM podl_bayutsa.dim_contracts dc
JOIN podl_bayutsa.dim_packages dp ON dc.package_sk = dp.package_sk
JOIN jarvis_empg.jarvis_empg__crm_contracts cc
    ON CAST(SPLIT_PART(dc.contract_sk, '|', 5) AS BIGINT) = cc.id
JOIN jarvis_empg.jarvis_empg__crm_users u ON cc.assignee_id = u.id
CROSS JOIN months m
WHERE cc.is_online = TRUE
  AND LOWER(dp.package_name_en) NOT IN ('unknown')
  AND u.email NOT IN ('online-user@jarvisempg.com', 'lms-user@jarvisempg.com')
  AND dc.date_sign_nk >= m.month_start AND dc.date_sign_nk < m.month_end
  AND m.month_num >= 1
  AND (dc.contract_category_sk NOT LIKE '%82')
  AND (cc.operation_type IS NULL OR cc.operation_type != 'delete')
GROUP BY m.month_num
ORDER BY m.month_num
""")
online_pkg_monthly = {}
for _, row in online_pkg_df.iterrows():
    online_pkg_monthly[int(row['month_num'])] = int(row['packages_sold'])
with open('C:/Users/hamza.rizwan/online_packages_monthly_2026.json', 'w') as f:
    json.dump({str(k): v for k, v in online_pkg_monthly.items()}, f, separators=(',', ':'))
print(f"  Online packages saved: {online_pkg_monthly}")

# ============================================================
# ONLINE ACTIVE LISTINGS SNAPSHOT (clients assigned to online-user)
# ============================================================
print("\nPulling online channel active listings...")
online_al_df = run_query("""
SELECT
    COUNT(DISTINCT l.listing_nk)  AS active_listings,
    COUNT(DISTINCT a.account_sk)  AS active_agencies
FROM podl_bayutsa.dim_accounts a
...
JOIN jarvis_empg.jarvis_empg__crm_users u
    ON u.id = cl.assignee_id
WHERE u.email = 'online-user@jarvisempg.com'
  AND l.listing_nk IS NOT NULL
""")
_online_al_row = online_al_df.iloc[0] if len(online_al_df) > 0 else None
online_listings_snapshot = {
    'total':    int(_online_al_row['active_listings']) if _online_al_row is not None else 0,
    'agencies': int(_online_al_row['active_agencies']) if _online_al_row is not None else 0,
}
with open('C:/Users/hamza.rizwan/online_listings_snapshot.json', 'w') as f:
    json.dump(online_listings_snapshot, f, separators=(',', ':'))
print(f"  Online listings snapshot saved: {online_listings_snapshot}")
